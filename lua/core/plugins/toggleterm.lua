return {
    ...
    -- Auto command to remove sign column on terminal
		...
		vim.keymap.set(
			"v",
			"<F9>",
			"<cmd>'<,'>lua _SEND_VIS_SELECTION_TO_IPYTHON()<CR><cmd>TermExec cmd=<CR>",
		...
		vim.keymap.set(
			"n",
			"<F9>",
			"<cmd>ToggleTermSendCurrentLine<CR><cmd>TermExec cmd=<CR>",
			{ desc = "term run current line" }
		)
		vim.keymap.set(
			"n",
			"<F5>",
			"<cmd>lua _RUN_FILE_IN_IPYTHON()<CR><cmd>TermExec cmd=<CR>",
			{ desc = "term run current line" }
		)
	end,
}
